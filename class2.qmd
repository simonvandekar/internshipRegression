...
model_interaction_smoke <- lm(fev ~ age * sex + smoke, data = fev)

summary(model_interaction)
summary(model_interaction_smoke)
```

* Interpretation of tests and type 2 sum of squares using `car::Anova`

```{r}
anova(model_interaction_smoke)
summary(aov(model_interaction_smoke))
library(car)
Anova(model_interaction_smoke, type = 2)
```
...
par(mfrow=c(2,1))
hist(fev$fev, xlab='FEV', main='Histogram of FEV')
hist(log(fev$fev), xlab='log(FEV)', main='Histogram of log(FEV)')
# Log-transform the response and re-fit the model
model_log <- lm(log(fev) ~ age + smoke + sex, data = fev)
summary(model_log)

ageGrid = ref_grid(model_log, at=list(age=seq(min(fev$age), max(fev$age), length.out=100)) )
predictions <- emmeans(ageGrid, ~age)

# Prepare data for plotting
pred_plot_data <- as.data.frame(predictions)
pred_plot_data[, c('emmean', 'lower.CL', 'upper.CL')] = exp(pred_plot_data[, c('emmean', 'lower.CL', 'upper.CL')])
...
* The theory for this is a bit complicated.
* These robust estimators can be a little unstable in small samples and with mixed models (random effects models)
...
```{r}
polyAge = poly(fev$age, 5)
plotData = as.data.frame(cbind(fev$age, polyAge))
names(plotData) = c('Age', 'polyTerm1', 'polyTerm2', 'polyTerm3', 'polyTerm4' ,'polyTerm5')
plot(plotData)

# Polynomial term for age
model_poly <- lm(fev ~ poly(age, 5) + smoke + sex, data = fev)
# linear model
model_linear <- lm(fev ~ age + smoke + sex, data = fev)
summary(model_poly)
library(car)
Anova(model_poly)
# allow you to directly compare these models
...
nsAge = ns(fev$age, df=5)
plotData = as.data.frame(cbind(fev$age, nsAge))
names(plotData) = c('Age', 'nsTerm1', 'nsTerm2', 'nsTerm3', 'nsTerm4' ,'nsTerm5')
plot(plotData)
...
* Plot regression diagnostics for this.

# Best model?

* What's likely the most reasonable model fit?