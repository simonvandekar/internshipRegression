...
- Prepping for presentations today
- Interpret logistic regression coefficients
- Evaluating model fit

# Presentations

- 1 slide/minute (including introduction)
- Better too short than too long, leave time for questions
- Describe figures very slowly -- Biostatistics covers diverse topics and our knowledge about different fields don't overlap
...
$$   \text{logit}(p) = \log\left(\frac{p}{1 - p}\right) = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \ldots + \beta_k X_k
   ...
   - \$ \beta\_1, \beta\_2, \ldots, \beta\_k \$ are the coefficients for the predictors.

## Intepreting coefficients on the log-odds scale

Each coefficient \$ \beta\_i \$ represents the change in the log odds of the outcome for a one-unit increase in the corresponding predictor variable \$ X_i \$, holding all other predictors constant.

- **For continuous predictors (e.g., Age)**:
  - If \$ \beta\_1 =-0.004\$ (age): for every 1-year increase in age, the log odds of being a smoker increase by 0.05.
- **For categorical predictors (e.g., Sex and Region)**:
  - If \$ \beta\_2 = 0.37\$, then being male decreases the log odds of being a smoker by 1.5.

## Interpreting coefficients on the odds scale

- To convert the log odds to odds, we exponentiate: \[ \text{odds} = e\^{\text{logit}(p)} = e\^{\beta\_0 + \beta\_1 X_1 + \beta\_2 X_2 + \ldots + \beta\_k X_k} \]

- **Odds Ratio (OR)**:

  - The odds ratio for a one-unit increase in \$ X_i \$ is given by: \[ \text{OR} = e\^{\beta\_i} \]
